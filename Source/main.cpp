...
int main(int argc, char *argv[])
{
    ...
    //    QObject::connect(video_reader0,SIGNAL(finishDraw(QImage)), video_viewer0, SLOT(onFinsihedDraw(QImage)));
    receiver.addVideoViewer(video_viewer0);// receive from socket
    QObject::connect(&receiver,SIGNAL(finishedDraw_0(QImage)), video_viewer0, SLOT(onFinsihedDraw(QImage)));
